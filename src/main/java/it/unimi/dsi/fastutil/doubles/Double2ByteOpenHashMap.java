...
public class Double2ByteOpenHashMap extends AbstractDouble2ByteMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Byte remove( final Object ok ) {
  ...
  if ( ( Double.doubleToLongBits(curr) == Double.doubleToLongBits(k) ) ) return (Byte.valueOf(removeEntry( pos )));
  while( true ) {
 ...
 public Byte get( final Double ok ) {
  ...
  if ( ( Double.doubleToLongBits(k) == 0 ) ) return containsNullKey ? (Byte.valueOf(value[ n ])) : (null);
  double curr;
  final double[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return (null);
  if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return (Byte.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2ByteMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Double.doubleToLongBits(k) == 0 ) ) return ( Double2ByteOpenHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).byteValue())) ) );
   double curr;
   final double[] key = Double2ByteOpenHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return false;
   if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).byteValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Double.doubleToLongBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).byteValue())) );
