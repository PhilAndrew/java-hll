...
public class Double2FloatOpenHashMap extends AbstractDouble2FloatMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Float remove( final Object ok ) {
  ...
  int pos;
  // The starting point.
  if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return (null);
  if ( ( Double.doubleToLongBits(curr) == Double.doubleToLongBits(k) ) ) return (Float.valueOf(removeEntry( pos )));
  while( true ) {
   if ( ( Double.doubleToLongBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return (null);
   if ( ( Double.doubleToLongBits(curr) == Double.doubleToLongBits(k) ) ) return (Float.valueOf(removeEntry( pos )));
  }
 }
 public Float get( final Double ok ) {
  final double k = ((ok).doubleValue());
  if ( ( Double.doubleToLongBits(k) == 0 ) ) return containsNullKey ? (Float.valueOf(value[ n ])) : (null);
  double curr;
  final double[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return (null);
  if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return (Float.valueOf(value[ pos ]));
 ...
 final class MapEntry implements Double2FloatMap.Entry , Map.Entry<Double, Float> {
  ...
  public boolean equals( final Object o ) {
   ...
   return ( Double.doubleToLongBits(key[ index ]) == Double.doubleToLongBits(((e.getKey()).doubleValue())) ) && ( (value[ index ]) == (((e.getValue()).floatValue())) );
  }
  public int hashCode() {
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2FloatMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Double.doubleToLongBits(k) == 0 ) ) return ( Double2FloatOpenHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).floatValue())) ) );
   double curr;
   final double[] key = Double2FloatOpenHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return false;
   if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).floatValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Double.doubleToLongBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).floatValue())) );
