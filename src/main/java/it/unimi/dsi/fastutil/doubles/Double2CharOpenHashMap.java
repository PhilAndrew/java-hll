...
public class Double2CharOpenHashMap extends AbstractDouble2CharMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Character remove( final Object ok ) {
  ...
  if ( ( Double.doubleToLongBits(curr) == Double.doubleToLongBits(k) ) ) return (Character.valueOf(removeEntry( pos )));
  while( true ) {
 ...
 public Character get( final Double ok ) {
  ...
  if ( ( Double.doubleToLongBits(k) == 0 ) ) return containsNullKey ? (Character.valueOf(value[ n ])) : (null);
  double curr;
  final double[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return (null);
  if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return (Character.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2CharMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Double.doubleToLongBits(k) == 0 ) ) return ( Double2CharOpenHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).charValue())) ) );
   double curr;
   final double[] key = Double2CharOpenHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return false;
   if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).charValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Double.doubleToLongBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).charValue())) );
