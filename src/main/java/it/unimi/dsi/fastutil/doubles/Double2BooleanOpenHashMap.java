...
public class Double2BooleanOpenHashMap extends AbstractDouble2BooleanMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Boolean remove( final Object ok ) {
  ...
  if ( ( Double.doubleToLongBits(curr) == Double.doubleToLongBits(k) ) ) return (Boolean.valueOf(removeEntry( pos )));
  while( true ) {
 ...
 public Boolean get( final Double ok ) {
  ...
  if ( ( Double.doubleToLongBits(k) == 0 ) ) return containsNullKey ? (Boolean.valueOf(value[ n ])) : (null);
  double curr;
  final double[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return (null);
  if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return (Boolean.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2BooleanMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Double.doubleToLongBits(k) == 0 ) ) return ( Double2BooleanOpenHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).booleanValue())) ) );
   double curr;
   final double[] key = Double2BooleanOpenHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return false;
   if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).booleanValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Double.doubleToLongBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).booleanValue())) );
