...
public class Double2ShortOpenHashMap extends AbstractDouble2ShortMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Short remove( final Object ok ) {
  ...
  if ( ( Double.doubleToLongBits(curr) == Double.doubleToLongBits(k) ) ) return (Short.valueOf(removeEntry( pos )));
  while( true ) {
 ...
 public Short get( final Double ok ) {
  ...
  if ( ( Double.doubleToLongBits(k) == 0 ) ) return containsNullKey ? (Short.valueOf(value[ n ])) : (null);
  double curr;
  final double[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return (null);
  if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return (Short.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2ShortMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Double.doubleToLongBits(k) == 0 ) ) return ( Double2ShortOpenHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).shortValue())) ) );
   double curr;
   final double[] key = Double2ShortOpenHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return false;
   if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).shortValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Double.doubleToLongBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).shortValue())) );
  ...
  public boolean remove( final Object o ) {
   ...
   final Map.Entry<Double, Short> e = (Map.Entry<Double, Short>)o;
   final double k = ((e.getKey()).doubleValue());
   final short v = ((e.getValue()).shortValue());
   if ( ( Double.doubleToLongBits(k) == 0 ) ) {
