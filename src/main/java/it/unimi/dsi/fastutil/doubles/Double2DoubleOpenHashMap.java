...
public class Double2DoubleOpenHashMap extends AbstractDouble2DoubleMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Double remove( final Object ok ) {
  ...
  if ( ( Double.doubleToLongBits(curr) == Double.doubleToLongBits(k) ) ) return (Double.valueOf(removeEntry( pos )));
  while( true ) {
 ...
 public Double get( final Double ok ) {
  ...
  if ( ( Double.doubleToLongBits(k) == 0 ) ) return containsNullKey ? (Double.valueOf(value[ n ])) : (null);
  double curr;
  final double[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return (null);
  if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return (Double.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Double2DoubleMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Double.doubleToLongBits(k) == 0 ) ) return ( Double2DoubleOpenHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).doubleValue())) ) );
   double curr;
   final double[] key = Double2DoubleOpenHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Double.doubleToLongBits(curr = key[ pos = (int)it.unimi.dsi.fastutil.HashCommon.mix( Double.doubleToRawLongBits(k) ) & mask ]) == 0 ) ) return false;
   if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).doubleValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Double.doubleToLongBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Double.doubleToLongBits(k) == Double.doubleToLongBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).doubleValue())) );
