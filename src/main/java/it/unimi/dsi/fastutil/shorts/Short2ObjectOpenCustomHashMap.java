...
public class Short2ObjectOpenCustomHashMap <V> extends AbstractShort2ObjectMap <V> implements java.io.Serializable, Cloneable, Hash {
 ...
 public V remove( final Object ok ) {
  ...
  int pos;
  // The starting point.
  if ( ( (curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == ((short)0) ) ) return (this.defRetValue);
  if ( ( (curr) == (k) ) ) return (removeEntry( pos ));
  while( true ) {
   if ( ( (curr = key[ pos = ( pos + 1 ) & mask ]) == ((short)0) ) ) return (this.defRetValue);
   if ( ( (curr) == (k) ) ) return (removeEntry( pos ));
  }
 }
 public V get( final Short ok ) {
  final short k = ((ok).shortValue());
  if ( ( (k) == ((short)0) ) ) return containsNullKey ? (value[ n ]) : (this.defRetValue);
  short curr;
  final short[] key = this.key;
  int pos;
  // The starting point.
 ...
 public boolean containsValue( final Object v ) {
  ...
  for( int i = n; i-- != 0; ) if ( ! ( (key[ i ]) == ((short)0) ) && ( (value[ i ]) == null ? (v) == null : (value[ i ]).equals(v) ) ) return true;
  return false;
 }
 ...
 private final class MapEntrySet extends AbstractObjectSet<Short2ObjectMap.Entry <V> > implements FastEntrySet <V> {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( (k) == ((short)0) ) ) return ( Short2ObjectOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == null ? ((e.getValue())) == null : (value[ n ]).equals((e.getValue())) ) );
   short curr;
   final short[] key = Short2ObjectOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
