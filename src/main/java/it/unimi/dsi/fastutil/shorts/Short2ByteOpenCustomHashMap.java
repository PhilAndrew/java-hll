...
public class Short2ByteOpenCustomHashMap extends AbstractShort2ByteMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Byte get( final Short ok ) {
  ...
  if ( ( (k) == ((short)0) ) ) return containsNullKey ? (Byte.valueOf(value[ n ])) : (null);
  short curr;
  final short[] key = this.key;
  int pos;
  // The starting point.
 ...
 private final class MapEntrySet extends AbstractObjectSet<Short2ByteMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( (k) == ((short)0) ) ) return ( Short2ByteOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).byteValue())) ) );
   short curr;
   final short[] key = Short2ByteOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( (curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == ((short)0) ) ) return false;
   if ( ( (k) == (curr) ) ) return ( (value[ pos ]) == (((e.getValue()).byteValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( (curr = key[ pos = ( pos + 1 ) & mask ]) == ((short)0) ) ) return false;
    if ( ( (k) == (curr) ) ) return ( (value[ pos ]) == (((e.getValue()).byteValue())) );
