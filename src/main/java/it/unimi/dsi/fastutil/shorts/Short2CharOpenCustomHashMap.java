...
public class Short2CharOpenCustomHashMap extends AbstractShort2CharMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Character put( final Short ok, final Character ov ) {
  ...
  final int pos = insert( ((ok).shortValue()), v );
 ...
 public Character remove( final Object ok ) {
  ...
  int pos;
  // The starting point.
  if ( ( (curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == ((short)0) ) ) return (null);
  if ( ( (curr) == (k) ) ) return (Character.valueOf(removeEntry( pos )));
  while( true ) {
   if ( ( (curr = key[ pos = ( pos + 1 ) & mask ]) == ((short)0) ) ) return (null);
   if ( ( (curr) == (k) ) ) return (Character.valueOf(removeEntry( pos )));
  }
 }
 public Character get( final Short ok ) {
  final short k = ((ok).shortValue());
  if ( ( (k) == ((short)0) ) ) return containsNullKey ? (Character.valueOf(value[ n ])) : (null);
  short curr;
  final short[] key = this.key;
  int pos;
  // The starting point.
  if ( ( (curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == ((short)0) ) ) return (null);
  if ( ( (k) == (curr) ) ) return (Character.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Short2CharMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( (k) == ((short)0) ) ) return ( Short2CharOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).charValue())) ) );
   short curr;
   final short[] key = Short2CharOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( (curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == ((short)0) ) ) return false;
   if ( ( (k) == (curr) ) ) return ( (value[ pos ]) == (((e.getValue()).charValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( (curr = key[ pos = ( pos + 1 ) & mask ]) == ((short)0) ) ) return false;
    if ( ( (k) == (curr) ) ) return ( (value[ pos ]) == (((e.getValue()).charValue())) );
