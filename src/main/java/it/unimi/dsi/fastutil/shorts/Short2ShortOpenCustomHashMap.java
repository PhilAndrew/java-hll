...
public class Short2ShortOpenCustomHashMap extends AbstractShort2ShortMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Short get( final Short ok ) {
  ...
  if ( ( (k) == ((short)0) ) ) return containsNullKey ? (Short.valueOf(value[ n ])) : (null);
  short curr;
  final short[] key = this.key;
  int pos;
  // The starting point.
 ...
 private final class MapEntrySet extends AbstractObjectSet<Short2ShortMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( (k) == ((short)0) ) ) return ( Short2ShortOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).shortValue())) ) );
   short curr;
   final short[] key = Short2ShortOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( (curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == ((short)0) ) ) return false;
   if ( ( (k) == (curr) ) ) return ( (value[ pos ]) == (((e.getValue()).shortValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( (curr = key[ pos = ( pos + 1 ) & mask ]) == ((short)0) ) ) return false;
    if ( ( (k) == (curr) ) ) return ( (value[ pos ]) == (((e.getValue()).shortValue())) );
