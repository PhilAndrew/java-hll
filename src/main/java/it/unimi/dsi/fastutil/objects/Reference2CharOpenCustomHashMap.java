...
public class Reference2CharOpenCustomHashMap <K> extends AbstractReference2CharMap <K> implements java.io.Serializable, Cloneable, Hash {
 ...
 public Character remove( final Object ok ) {
  ...
  int pos;
  // The starting point.
  if ( ( (curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( System.identityHashCode(k) ) ) & mask ]) == (null) ) ) return (null);
 ...
 private class MapIterator {
  ...
  private final void shiftKeys( int pos ) {
   ...
   for(;;) {
    ...
    if ( pos < last ) { // Wrapped entry.
     if ( wrapped == null ) wrapped = new ReferenceArrayList <K>( 2 );
     wrapped.add( key[ pos ] );
    }
    key[ last ] = curr;
    value[ last ] = value[ pos ];
  ...
  public void remove() {
   ...
   if ( last == n ) {
    ...
    Reference2CharOpenCustomHashMap.this.remove( wrapped.set( - pos - 1, null ) );
 ...
 private final class MapEntrySet extends AbstractObjectSet<Reference2CharMap.Entry <K> > implements FastEntrySet <K> {
  ...
  public boolean contains( final Object o ) {
   ...
   final K k = (e.getKey());
   if ( ( (k) == (null) ) ) return ( Reference2CharOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).charValue())) ) );
   K curr;
   final K[] key = Reference2CharOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
