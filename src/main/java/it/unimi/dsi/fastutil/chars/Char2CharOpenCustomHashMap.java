...
public class Char2CharOpenCustomHashMap extends AbstractChar2CharMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Character remove( final Object ok ) {
  ...
  int pos;
  // The starting point.
  if ( ( (curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == ((char)0) ) ) return (null);
 ...
 public Character get( final Character ok ) {
  ...
  if ( ( (k) == ((char)0) ) ) return containsNullKey ? (Character.valueOf(value[ n ])) : (null);
  char curr;
  final char[] key = this.key;
  int pos;
  // The starting point.
 ...
 private final class MapEntrySet extends AbstractObjectSet<Char2CharMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( (k) == ((char)0) ) ) return ( Char2CharOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).charValue())) ) );
   char curr;
   final char[] key = Char2CharOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
