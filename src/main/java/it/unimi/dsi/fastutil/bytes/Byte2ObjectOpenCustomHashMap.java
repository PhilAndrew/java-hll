...
public class Byte2ObjectOpenCustomHashMap <V> extends AbstractByte2ObjectMap <V> implements java.io.Serializable, Cloneable, Hash {
 ...
 public V remove( final Object ok ) {
  ...
  int pos;
  // The starting point.
  if ( ( (curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == ((byte)0) ) ) return (this.defRetValue);
  if ( ( (curr) == (k) ) ) return (removeEntry( pos ));
  while( true ) {
   if ( ( (curr = key[ pos = ( pos + 1 ) & mask ]) == ((byte)0) ) ) return (this.defRetValue);
   if ( ( (curr) == (k) ) ) return (removeEntry( pos ));
  }
 }
 public V get( final Byte ok ) {
  final byte k = ((ok).byteValue());
  if ( ( (k) == ((byte)0) ) ) return containsNullKey ? (value[ n ]) : (this.defRetValue);
  byte curr;
  final byte[] key = this.key;
  int pos;
  // The starting point.
 ...
 public boolean containsValue( final Object v ) {
  ...
  for( int i = n; i-- != 0; ) if ( ! ( (key[ i ]) == ((byte)0) ) && ( (value[ i ]) == null ? (v) == null : (value[ i ]).equals(v) ) ) return true;
  return false;
 }
 ...
 private final class MapEntrySet extends AbstractObjectSet<Byte2ObjectMap.Entry <V> > implements FastEntrySet <V> {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( (k) == ((byte)0) ) ) return ( Byte2ObjectOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == null ? ((e.getValue())) == null : (value[ n ]).equals((e.getValue())) ) );
   byte curr;
   final byte[] key = Byte2ObjectOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
