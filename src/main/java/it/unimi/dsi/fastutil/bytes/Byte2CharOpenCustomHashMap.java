...
public class Byte2CharOpenCustomHashMap extends AbstractByte2CharMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Character put( final Byte ok, final Character ov ) {
  ...
  final int pos = insert( ((ok).byteValue()), v );
 ...
 public Character remove( final Object ok ) {
  ...
  int pos;
  // The starting point.
  if ( ( (curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == ((byte)0) ) ) return (null);
  if ( ( (curr) == (k) ) ) return (Character.valueOf(removeEntry( pos )));
  while( true ) {
   if ( ( (curr = key[ pos = ( pos + 1 ) & mask ]) == ((byte)0) ) ) return (null);
   if ( ( (curr) == (k) ) ) return (Character.valueOf(removeEntry( pos )));
  }
 }
 public Character get( final Byte ok ) {
  final byte k = ((ok).byteValue());
  if ( ( (k) == ((byte)0) ) ) return containsNullKey ? (Character.valueOf(value[ n ])) : (null);
  byte curr;
  final byte[] key = this.key;
  int pos;
  // The starting point.
 ...
 private final class MapEntrySet extends AbstractObjectSet<Byte2CharMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( (k) == ((byte)0) ) ) return ( Byte2CharOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).charValue())) ) );
   byte curr;
   final byte[] key = Byte2CharOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
