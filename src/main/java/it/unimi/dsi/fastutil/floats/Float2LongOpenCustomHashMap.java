...
public class Float2LongOpenCustomHashMap extends AbstractFloat2LongMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Long remove( final Object ok ) {
  ...
  if ( ( Float.floatToIntBits(curr) == Float.floatToIntBits(k) ) ) return (Long.valueOf(removeEntry( pos )));
  while( true ) {
 ...
 public Long get( final Float ok ) {
  ...
  if ( ( Float.floatToIntBits(k) == 0 ) ) return containsNullKey ? (Long.valueOf(value[ n ])) : (null);
  float curr;
  final float[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Float.floatToIntBits(curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == 0 ) ) return (null);
  if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return (Long.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2LongMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Float.floatToIntBits(k) == 0 ) ) return ( Float2LongOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).longValue())) ) );
   float curr;
   final float[] key = Float2LongOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Float.floatToIntBits(curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == 0 ) ) return false;
   if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).longValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Float.floatToIntBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).longValue())) );
