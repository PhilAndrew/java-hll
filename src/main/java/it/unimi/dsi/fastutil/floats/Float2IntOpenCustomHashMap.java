...
public class Float2IntOpenCustomHashMap extends AbstractFloat2IntMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Integer remove( final Object ok ) {
  ...
  if ( ( Float.floatToIntBits(curr) == Float.floatToIntBits(k) ) ) return (Integer.valueOf(removeEntry( pos )));
  while( true ) {
 ...
 public Integer get( final Float ok ) {
  ...
  if ( ( Float.floatToIntBits(k) == 0 ) ) return containsNullKey ? (Integer.valueOf(value[ n ])) : (null);
  float curr;
  final float[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Float.floatToIntBits(curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == 0 ) ) return (null);
  if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return (Integer.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2IntMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Float.floatToIntBits(k) == 0 ) ) return ( Float2IntOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).intValue())) ) );
   float curr;
   final float[] key = Float2IntOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Float.floatToIntBits(curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == 0 ) ) return false;
   if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).intValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Float.floatToIntBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).intValue())) );
