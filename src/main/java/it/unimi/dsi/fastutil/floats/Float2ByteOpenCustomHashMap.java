...
public class Float2ByteOpenCustomHashMap extends AbstractFloat2ByteMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Byte remove( final Object ok ) {
  ...
  if ( ( Float.floatToIntBits(curr) == Float.floatToIntBits(k) ) ) return (Byte.valueOf(removeEntry( pos )));
  while( true ) {
 ...
 public Byte get( final Float ok ) {
  ...
  if ( ( Float.floatToIntBits(k) == 0 ) ) return containsNullKey ? (Byte.valueOf(value[ n ])) : (null);
  float curr;
  final float[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Float.floatToIntBits(curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == 0 ) ) return (null);
  if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return (Byte.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2ByteMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Float.floatToIntBits(k) == 0 ) ) return ( Float2ByteOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).byteValue())) ) );
   float curr;
   final float[] key = Float2ByteOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Float.floatToIntBits(curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == 0 ) ) return false;
   if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).byteValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Float.floatToIntBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).byteValue())) );
