...
public class Float2BooleanOpenCustomHashMap extends AbstractFloat2BooleanMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Boolean remove( final Object ok ) {
  ...
  if ( ( Float.floatToIntBits(curr) == Float.floatToIntBits(k) ) ) return (Boolean.valueOf(removeEntry( pos )));
  while( true ) {
 ...
 public Boolean get( final Float ok ) {
  ...
  if ( ( Float.floatToIntBits(k) == 0 ) ) return containsNullKey ? (Boolean.valueOf(value[ n ])) : (null);
  float curr;
  final float[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Float.floatToIntBits(curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == 0 ) ) return (null);
  if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return (Boolean.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2BooleanMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Float.floatToIntBits(k) == 0 ) ) return ( Float2BooleanOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).booleanValue())) ) );
   float curr;
   final float[] key = Float2BooleanOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Float.floatToIntBits(curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == 0 ) ) return false;
   if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).booleanValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Float.floatToIntBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).booleanValue())) );
