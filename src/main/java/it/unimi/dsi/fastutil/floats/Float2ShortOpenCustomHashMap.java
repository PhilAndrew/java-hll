...
public class Float2ShortOpenCustomHashMap extends AbstractFloat2ShortMap implements java.io.Serializable, Cloneable, Hash {
 ...
 public Short remove( final Object ok ) {
  ...
  if ( ( Float.floatToIntBits(curr) == Float.floatToIntBits(k) ) ) return (Short.valueOf(removeEntry( pos )));
  while( true ) {
 ...
 public Short get( final Float ok ) {
  ...
  if ( ( Float.floatToIntBits(k) == 0 ) ) return containsNullKey ? (Short.valueOf(value[ n ])) : (null);
  float curr;
  final float[] key = this.key;
  int pos;
  // The starting point.
  if ( ( Float.floatToIntBits(curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == 0 ) ) return (null);
  if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return (Short.valueOf(value[ pos ]));
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2ShortMap.Entry > implements FastEntrySet {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Float.floatToIntBits(k) == 0 ) ) return ( Float2ShortOpenCustomHashMap.this.containsNullKey && ( (value[ n ]) == (((e.getValue()).shortValue())) ) );
   float curr;
   final float[] key = Float2ShortOpenCustomHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Float.floatToIntBits(curr = key[ pos = ( it.unimi.dsi.fastutil.HashCommon.mix( strategy.hashCode(k) ) ) & mask ]) == 0 ) ) return false;
   if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).shortValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Float.floatToIntBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == (((e.getValue()).shortValue())) );
