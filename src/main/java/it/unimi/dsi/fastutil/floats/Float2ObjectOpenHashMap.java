...
public class Float2ObjectOpenHashMap <V> extends AbstractFloat2ObjectMap <V> implements java.io.Serializable, Cloneable, Hash {
 ...
 public V remove( final Object ok ) {
  ...
  int pos;
  // The starting point.
  if ( ( Float.floatToIntBits(curr = key[ pos = it.unimi.dsi.fastutil.HashCommon.mix( it.unimi.dsi.fastutil.HashCommon.float2int(k) ) & mask ]) == 0 ) ) return (this.defRetValue);
  if ( ( Float.floatToIntBits(curr) == Float.floatToIntBits(k) ) ) return (removeEntry( pos ));
  while( true ) {
   if ( ( Float.floatToIntBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return (this.defRetValue);
   if ( ( Float.floatToIntBits(curr) == Float.floatToIntBits(k) ) ) return (removeEntry( pos ));
  }
 }
 public V get( final Float ok ) {
  final float k = ((ok).floatValue());
  if ( ( Float.floatToIntBits(k) == 0 ) ) return containsNullKey ? (value[ n ]) : (this.defRetValue);
  float curr;
  final float[] key = this.key;
  int pos;
  // The starting point.
 ...
 public boolean containsValue( final Object v ) {
  ...
  for( int i = n; i-- != 0; ) if ( ! ( Float.floatToIntBits(key[ i ]) == 0 ) && ( (value[ i ]) == null ? (v) == null : (value[ i ]).equals(v) ) ) return true;
  return false;
 }
 ...
 final class MapEntry implements Float2ObjectMap.Entry <V>, Map.Entry<Float, V> {
  ...
  public boolean equals( final Object o ) {
   ...
   return ( Float.floatToIntBits(key[ index ]) == Float.floatToIntBits(((e.getKey()).floatValue())) ) && ( (value[ index ]) == null ? ((e.getValue())) == null : (value[ index ]).equals((e.getValue())) );
  }
  public int hashCode() {
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2ObjectMap.Entry <V> > implements FastEntrySet <V> {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Float.floatToIntBits(k) == 0 ) ) return ( Float2ObjectOpenHashMap.this.containsNullKey && ( (value[ n ]) == null ? ((e.getValue())) == null : (value[ n ]).equals((e.getValue())) ) );
   float curr;
   final float[] key = Float2ObjectOpenHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Float.floatToIntBits(curr = key[ pos = it.unimi.dsi.fastutil.HashCommon.mix( it.unimi.dsi.fastutil.HashCommon.float2int(k) ) & mask ]) == 0 ) ) return false;
   if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == null ? ((e.getValue())) == null : (value[ pos ]).equals((e.getValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Float.floatToIntBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == null ? ((e.getValue())) == null : (value[ pos ]).equals((e.getValue())) );
