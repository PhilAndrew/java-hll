...
public class Float2ReferenceOpenHashMap <V> extends AbstractFloat2ReferenceMap <V> implements java.io.Serializable, Cloneable, Hash {
 ...
 private final class MapEntrySet extends AbstractObjectSet<Float2ReferenceMap.Entry <V> > implements FastEntrySet <V> {
  ...
  public boolean contains( final Object o ) {
   ...
   if ( ( Float.floatToIntBits(k) == 0 ) ) return ( Float2ReferenceOpenHashMap.this.containsNullKey && ( (value[ n ]) == ((e.getValue())) ) );
   float curr;
   final float[] key = Float2ReferenceOpenHashMap.this.key;
   int pos;
   // The starting point.
   if ( ( Float.floatToIntBits(curr = key[ pos = it.unimi.dsi.fastutil.HashCommon.mix( it.unimi.dsi.fastutil.HashCommon.float2int(k) ) & mask ]) == 0 ) ) return false;
   if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == ((e.getValue())) );
   // There's always an unused entry.
   while( true ) {
    if ( ( Float.floatToIntBits(curr = key[ pos = ( pos + 1 ) & mask ]) == 0 ) ) return false;
    if ( ( Float.floatToIntBits(k) == Float.floatToIntBits(curr) ) ) return ( (value[ pos ]) == ((e.getValue())) );
